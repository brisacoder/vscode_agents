...
1. **Section IDs** — the agent's review categories (e.g. `F`, `I`, `A`, `P`, `C`, `S`, `L`, `U`, `PY` for Python Expert; `LC.atomicity`, `LC.invariants`, `LC.check-then-act`, `LC.idempotency`, `LC.boundary` for Logic & Correctness; `FA.deps`, `FA.async`, `FA.response`, `FA.middleware`, `FA.security`, `FA.background`, `FA.routing` for FastAPI; etc.).
2. **Hunter personas** — the diverse-prior reviewers (e.g. The Pessimist, The Adversary, The Scaler, The Maintainer, The Beginner, The Pythonista; or The Corruptor, The Racer, The Retrier, The Edge Finder; or whatever the agent has defined).
...
2. **Hunter roster** — the named hunter personas with their owned sections and one-line prior. The roster size is determined by the agent (Python Expert uses six; Logic & Correctness Expert uses four; smaller agents use as few as two), but every hunter must have a unique prior and at least one owned section.
