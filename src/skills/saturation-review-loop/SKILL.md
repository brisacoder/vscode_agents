...
2. **Hunter roster** — the named hunter personas with their owned sections and one-line prior. The roster size is determined by the agent (Python Expert uses six; Logic & Correctness Expert uses four; the Code Review Generalist uses three lens-hunters; smaller agents use as few as two), but every hunter must have a unique prior and at least one owned section. For a checklist-free generalist the "sections" are reading angles rather than domains, and the hunters are distinguished by stance (intent vs. behavior-on-input vs. literal tokens) rather than by subject area.
