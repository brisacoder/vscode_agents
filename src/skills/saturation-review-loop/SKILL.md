...
description: The canonical three-round, three-phase saturation loop that every review-mode specialist agent uses to drive its findings to zero-delta closure. Load whenever the agent is in Review mode (auditing a path, module, package, graph, Dockerfile, workflow, or other artifact) and needs a complete, idempotent findings report that does not change on re-runs. Cures the anti-pattern where a second run finds more issues than the first: Phase A (Verify) re-reads existing findings and renders Confirmed / Improved / Disproved; Phase B (Hunt) launches diverse-prior hunter subagents that draft independently before deduping; Phase C (Pattern propagation) searches sibling sites for the same anti-pattern. Three rounds max; terminates on the first zero-delta round. Each agent supplies its own section IDs and hunter-persona roster as inputs to the loop -- the loop mechanics themselves are uniform across agents and must not be paraphrased or weakened.
