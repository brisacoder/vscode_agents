...
description: "The mandatory contract for shepherding a submitted GitHub-native stack all the way to merged. Load at startup in every PR-lifecycle agent (PR Review Resolver, PR Watch Agent, PR Stack Planner) as the single source of truth for six rules their longer bodies must never contradict. Six rules: (1) set auto-merge on EVERY PR the moment a stack is created or submitted; (2) address ALL review comments across the WHOLE stack by default (Copilot, other LLM reviewers, humans) unasked; (3) review is iterative -- loop until a poll yields zero unaddressed comments AND zero open PRs; (4) when a comment is addressed by a code change and/or reply, RESOLVE its conversation; (5) every PR carries a Jira ticket key in plain ASCII (e.g. OCTO-1234); (6) submitted-and-green is NOT done -- merged is done, so babysit the stack until every PR is merged or closed. This skill owns the WHAT and WHEN; github-stacking owns the gh stack HOW."
