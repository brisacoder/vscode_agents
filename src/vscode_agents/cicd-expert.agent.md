...
name: "CI/CD Expert"
argument-hint: "Path to .github/workflows/*.yml. Optional: 'review only', 'rewrite'."
...
- `uv` usage inside Docker image layers → **Docker Expert**; this agent owns `uv` only as it appears *in workflow steps* (e.g. `run: uv sync --frozen` in a `.github/workflows/*.yml` job).
