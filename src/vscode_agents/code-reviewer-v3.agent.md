...
7. **Memory-tool ban for orchestration state.** This is a hard rule. The VS Code memory tool (any path under `/memories/` -- including `~/.vscode-server/.../GitHub.copilot-chat/memory-tool/memories/...`) is **forbidden** as a substitute for the on-disk JSON ledger. If you find yourself reading or writing `pr-<NNNN>-review-plan.md`, `review-plan.md`, `dispatch-state.md`, or any other ledger-shaped file inside `/memories/`, you are violating this rule. Symptoms include the human report freezing at "0 of N complete" while you keep doing real work, because the rendered report only ever reads `./pr_reviews/.code-review-ledger-...json` and that file is empty. Concrete rules:
    ...
    - If you discover a stale memory-tool ledger residue file when you start (e.g. you read your memory index and see `pr-<NNNN>-review-plan.md`), **delete it** before continuing. Do not migrate it. Do not consult it. Re-derive the truth from the JSON ledger only.
...
8. **Verify every ledger write before claiming it.** A write that did not land is a write that did not happen, and the report being frozen at "0 of N" is the user-visible signature of that failure. After every atomic `.tmp` + `mv` write to the ledger:
    1. Re-read the file path that should now exist.
    2. Parse it as JSON.
    3. Confirm the specific row(s) you just modified are in the expected state (e.g. the row you marked `running` reads back as `running` with the expected `started_utc`).
    4. Confirm `last_update_utc` is the new timestamp you wrote, not a stale one and not `00:00:00Z`.
    5. Confirm `summary.specialists_done + summary.specialists_failed + summary.specialists_running + summary.specialists_pending == specialists_total`.
    If any of those checks fail, the write did not land. Log `ledger-write-verify-failed: <reason>`, retry the write exactly once, and if the second attempt also fails, exit with `code-reviewer-v3: cannot persist ledger at <path>; refusing to continue with in-memory state.` The user must see the failure; you must not paper over it by continuing to dispatch and dropping results on the floor.
...
10. **One specialist per write -- never batch ledger updates.** When two or three specialists return in close succession (common with the 9-slot scheduler), it is tempting to merge their results into a single ledger write. Do not. Each specialist gets its own complete cycle: receive result -> update dispatch_table row -> stream findings -> update findings_index -> update clusters -> recompute summary from the dispatch_table (do NOT increment; recompute) -> atomic write -> verify -> rewrite human report. Then start on the next specialist. Batching invites the model to lose its place halfway through merging JSON arrays and silently drop the second and third specialists. The per-specialist cycle is short and disk writes are cheap; the cost of one extra ledger write is far less than the cost of a dropped result.
...
5. **Resume or initialise the ledger.** Check `./pr_reviews/.code-review-ledger-<sanitized-path>-<YYYY-MM-DD>.json`. If it exists, load it and treat every `done` row as already complete. If it has rows in state `running`, mark them `pending` and re-dispatch them (they were in flight when the previous session died). If the file does not exist, create it with one row per triggered (specialist, model) pair in state `pending`, write it atomically, and write an initial human report ("Review in progress: 0 of N specialists complete") to the report path so the user can already point readers at it. See `## Durable ledger format` below for the schema.
   ...
   **At the same moment, scan memory-tool storage for residue.** If you see files matching `pr-*-review-plan.md`, `review-plan-*.md`, `dispatch-state-*.md`, `code-review-*.md`, or anything else that looks like ledger-shaped review state in your `/memories/` listing (including subpaths under `~/.vscode-server/.../GitHub.copilot-chat/memory-tool/memories/`), delete them with the memory tool's delete command before proceeding. They are stale from previous sessions that violated rule 7. Re-derive every fact from the JSON ledger only. Do not consult them, do not migrate their content.
6. **Dispatch (bounded rolling window, memory-safe).** Walk the ledger's pending rows. Build the work queue. Do **not** launch all of them at once -- that has caused out-of-memory failures. Instead use a bounded rolling window:
   ...
   - **Each START is its own ledger transaction.** When you launch a specialist via the agent tool, in the SAME assistant turn do all of: (1) set the row's `state` to `running`; (2) set `started_utc` to the current UTC clock; (3) atomic-write the ledger; (4) re-read and verify per Constraint 8; (5) re-render the human report from the ledger so the Dispatch Summary State column shows `running`. Only then issue the next START or move on. Do not start three specialists, then update the ledger once -- that is a Constraint 10 violation that loses started_utc on the first two.
   - **Each FINISH is its own ledger transaction.** When a specialist returns, in the SAME assistant turn do all of: (1) update the row to `done` (or `failed`) with `finished_utc`, `findings_file`, and `raw_findings`; (2) run incremental assembly per step 7 sub-steps a through i; (3) re-render the human report from the ledger; (4) verify the report file on disk now shows the new specialist as `done` in the Dispatch Summary. Only then start the next pending row for the same model. If the next specialist is already returning while you are still mid-cycle, queue its result -- do not interleave it with the current cycle's writes.
...
7. **Incremental assembly (runs after every specialist returns).** This is the core change. Do **not** wait for all specialists to finish. After every single ledger update, do the following, in order, then rewrite the human report from the resulting ledger:
   ...
   h. **Verify the write landed.** Re-read the ledger file. Parse the JSON. Confirm the just-modified row is in the new state and that `last_update_utc` is the timestamp you just wrote. If verification fails, retry the write once; on second failure, exit per Constraint 8.
   ...
   j. **Verify the report rewrite.** Re-read the report file. Confirm the new Status line, the new Last update timestamp, and the affected specialist's Dispatch Summary State column all reflect the just-completed transition. If the report on disk does not match the ledger, the rewrite did not land -- retry once, then exit per Constraint 8.
