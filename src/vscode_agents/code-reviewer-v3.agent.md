...
6. **Dispatch (bounded rolling window, parallel within the window).** Walk the ledger's pending rows. Build the work queue. Then dispatch in parallel, bounded by a 9-slot window:
   ...
   - **Fan out STARTs in parallel.** Issue all 9 initial subagent invocations as parallel agent-tool calls -- the runtime supports concurrent subagents and the whole point of the rolling window is to use them. Do NOT serialize the STARTs one per assistant turn. After the fan-out, write a single ledger update that flips all seeded rows from `pending` to `running` with their `started_utc` timestamps. See Constraint 10 for why batched STARTs are correct and required.
