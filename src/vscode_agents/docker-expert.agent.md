...
name: "Docker Expert"
argument-hint: "Path to Dockerfile / docker-compose.yml / .dockerignore. Optional: 'review only', 'rewrite'."
...
- `uv` usage in CI/CD workflow steps → **CI/CD Expert**; this agent owns `uv` only as it appears *inside image layers* (e.g. `RUN uv sync --frozen` in a Dockerfile).
