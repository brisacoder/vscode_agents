...
- Subagent A: `FA.deps` and `FA.async` — dependency injection lifecycle, `yield`-based cleanup, blocking calls in async, missing `await`.
- Subagent B: `FA.response` and `FA.routing` — response models, ORM-to-schema leakage, path ordering, mixed startup/shutdown mechanisms.
- Subagent C: `FA.middleware`, `FA.security`, `FA.background` — CORS ordering, untrusted proxy headers, JWT scope enforcement, `BackgroundTasks` durability.

For any finding whose recommended fix cites a FastAPI or Starlette API, fetch current upstream docs for the **pinned `fastapi` / `starlette` versions** from `uv.lock`. Treat training-data knowledge as suspect.
...
For every new finding, search the codebase for the same pattern at other route/dependency/middleware definitions using `search/textSearch` and `search/usages`. Each additional instance is its own finding.
