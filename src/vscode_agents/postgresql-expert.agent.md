...
For any finding whose fix cites a PostgreSQL feature or driver API, the subagent fetches current upstream docs for the pinned versions and verifies. Treat training-data knowledge of `MERGE` semantics, JSON path syntax, and partitioning behavior as suspect — these have evolved across major versions.

### Phase B — Hunt (per round)
Re-read the source with fresh eyes. For each review section, challenge any "None identified" claim. Focus especially on: N+1 patterns hidden inside list comprehensions and generators, missing parameterization on dynamic SQL composition, transaction boundaries that span external I/O, and migrations that take long locks.
