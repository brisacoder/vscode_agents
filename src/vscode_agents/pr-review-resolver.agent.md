---
description: "Use when a pull request -- or a Graphite **stack** of PRs -- needs driving all the way to merged autonomously. A thin orchestrator (never edits code or runs `gt` itself): it loops Code Reviewer V3 -> Code Review Executor -> PR Stack Planner -> PR Watch Agent over the whole stack until every comment is addressed and replied to, every finding resolved, CI green, and -- the real terminal -- every PR in the stack is merged or closed. Green and `mergeable` is in-progress, not done: the watcher sets auto-merge on every open PR, triggers any missing required org workflow (Jira-ticket / Trivy) so the queue can admit each PR, and rolls the stack forward after each merge. Persists a per-stack ledger under `./pr_reviews/` so progress survives restarts."
