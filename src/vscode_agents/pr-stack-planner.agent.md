...
10. **Submit the stack.** Run `gt submit --stack` (agent runs: add `--no-edit`; drafts: `--draft`; preview: `--dry-run`). This restack-validates, force-pushes each branch with lease, and creates/updates one PR per branch with the correct base. For each PR, ensure the description includes:
    ...
    - **A Jira ticket key in plain ASCII** (`[A-Z][A-Z0-9]+-[0-9]+`, e.g. `OCTO-1234`) in the title or body — the org-required `Require Jira Ticket` workflow blocks the merge queue without it. If no ticket is known, stop and ask the user; never invent one.
...
9a. **Verify a Jira ticket key is present.** The PR title or body must contain a plain-ASCII Jira key matching `[A-Z][A-Z0-9]+-[0-9]+` (e.g. `OCTO-1234`). The org-required `Require Jira Ticket` workflow blocks the merge queue without one. Missing key is `PR-no-jira-ticket` as **High**. The planner never invents a ticket number — if none is known, the finding is awaiting-user.
9b. **Verify auto-merge is enabled.** Every open PR in the stack must have `autoMergeRequest` set (`gh pr view <pr> --json autoMergeRequest`). Auto-merge is mandatory so the merge queue drains the stack. Any open PR without it is `PR-auto-merge-not-set` as **High**.
9c. **Verify required org workflows ran on the head SHA.** For each open PR, confirm the required org-level workflows the merge queue gates on (e.g. `Require Jira Ticket`, `Trivy Scan`) are present in `statusCheckRollup` for the current head SHA. A `mergeable` PR whose required org workflow is **absent** for the head SHA is `PR-org-workflow-not-run` as **High** — the queue will never admit it until the workflow runs.
...
| `PR-stuck-missing-check` | A branch is `BLOCKED`/`UNSTABLE` on a required check that is **absent** from `statusCheckRollup` for the current (stable) head SHA -- a Graphite restack force-push did not re-fire the org-level / required workflow (see the `graphite-stacking` skill, *Restack cascades and stuck PRs*). Nothing failed and nothing conflicts; the check never started. | **High** | Re-trigger, least-invasive first: `gh workflow run <workflow> --ref <branch>` or `gh run rerun <run-id>`. If the workflow only fires on a new SHA, add an empty re-trigger commit on the branch (`gt modify -c -m "chore: re-trigger CI"`) and `gt submit --stack`. Cap at 2 re-trigger attempts, then escalate to the user. Do NOT re-run a code fix. |
...
| `PR-auto-merge-not-set` | An open PR in the stack has no `autoMergeRequest` (auto-merge not enabled). Auto-merge is mandatory on every PR in the stack so the merge queue can drain it. | **High** | Enable auto-merge on every open PR: `gh pr merge <pr> --auto --squash` for each. Re-assert after every `gt submit --stack` (a force-push clears it). |
| `PR-org-workflow-not-run` | An open PR is `mergeable` but a required **org-level** workflow the merge queue gates on (e.g. `Require Jira Ticket`, `Trivy Scan`) is **absent** from `statusCheckRollup` for the current head SHA, so the queue will never admit it. (Distinct from `PR-stuck-missing-check`, which is the Graphite-restack-did-not-re-fire case; the recovery is the same.) | **High** | Trigger the workflow autonomously, least-invasive first: `gh workflow run <workflow> --ref <branch>` or `gh run rerun <run-id>`. If it only fires on a new commit, push a **no-op amend** (`gt modify -c -m "chore: re-trigger required workflows"`) and `gt submit --stack`. Cap 2 attempts, then escalate. Never wait for a human to notice. |
...
- `PR-auto-merge-not-set` -> enable auto-merge on every open PR in the stack: `gh pr merge <pr> --auto --squash` for each. Re-assert after any `gt submit --stack`. This is the mandatory drain mechanism, not a force-merge.
