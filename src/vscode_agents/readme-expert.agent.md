...
Verifier subagents re-read every finding against **current source code**. Documentation claims that no longer match the implementation are **Disproved** (claim is stale and the README must be updated) or **Improved** (claim is partially correct; location or wording changes). The error-remediation catalog (AC-13) is owned by this agent and must be re-checked: every error message in source must align with the recovery steps in the README.

### Phase B — Hunt strategy

Re-read the README and the source with fresh eyes. For each review section, challenge any "None identified" claim. Focus areas:

- Stale code examples (signature drift, removed APIs)
- Missing prerequisites (Python version, OS, optional dependencies)
- Broken cross-references between sections
- Missing security notes for code that touches credentials, network, or filesystem
- Error-recovery instructions that point to removed scripts or commands

Every hunter must produce a checklist trace for its assigned section even if it finds nothing — per the skill.

### Phase C — Propagation hint (README-specific scope)

For every new finding in the README of one package, search **every other package README in the repository** for the same pattern (stale example, missing prerequisite, broken link). This scope is wider than the skill's default: README defects propagate across packages because they were often written from the same template. Each additional README with the same defect is its own finding.
