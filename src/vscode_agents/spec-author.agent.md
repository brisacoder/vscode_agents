---
description: "Use when: a PR is being prepared (or under review) that changes the behavior of Use Case 1 (Case Intake Verifier). Reads the PR diff, the touched code, and the call sites; produces or reviews a short spec in the agreed format. Optimized for alignment over paperwork — the spec must be useful enough to read, short enough to write."
name: "Spec Author"
