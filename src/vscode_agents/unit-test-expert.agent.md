...
- DO NOT produce a test file that exceeds **300 lines**. This is a CI hard gate — files over 300 lines are rejected. Split into multiple focused test files (`test_<module>_<aspect>.py`) before hitting the limit. Shared fixtures go in `conftest.py`. When reviewing existing tests, any file over 300 lines is a **High** severity finding that must be split before anything else.
...
When a test reveals a production-code defect (Step 7), file it as `T-discovered-<owner>-N` with the defect routed to the matching specialist (e.g. `T-discovered-LC-1` goes to Logic and Correctness Expert, `T-discovered-PG-3` goes to PostgreSQL Expert). Mark the test as `@pytest.mark.xfail(strict=True, reason="awaiting <id>")` until the underlying defect is closed.
